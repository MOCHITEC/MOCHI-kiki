...
MVP は 2 機能で構成しています。1 つは **仕様補完**で、仕様確認発言を検知して社内ドキュメントを引き、200 字の回答を会議チャットへ proactive 投稿します。もう 1 つは **ライブ議事録ビュー**で、会議の発話から議事録とタイムラインをライブ生成し、管理コンソールで閲覧できます (会議チャットには投稿しません)。
...
本作品はハッカソン用のデモ環境としてこの構成を許容しましたが、社内本番に出すならデータ保管ポリシーやデータリージョンの確認が必要です。bot session 単位の従量課金も発生します。
...
5. **ChatPoster サービス** が proactive message として会議チャットへ投稿

オーケストレーターは `Orchestrator.process()` です。途中で「検索ヒット 0 件」「会議が登録されていない」のケースは早期 return で投稿せず、`[仕様補完]` プレフィックスは Python 側で付与しています。
...
| 文字起こし | utterance の生表示 | 5 秒ポーリング |

### 会議チャットへ投稿しない理由

仕様補完が「会議で全員に見せる前提」だったのに対し、議事録は「あとから振り返る前提」と用途が違うため別 UI に分けました。proactive message で議論を汚さない動線分離です。後段の致命欠点 1 (レイテンシ) とも噛み合う設計で、即時性を諦めて非同期に振った形でもあります。
...
- 出力形式は JSON か固定テンプレートに寄せる (後段の Python が parse・加工しやすい)
...
もう 1 つの工夫は **目印プレフィックスを LLM に書かせない** ことです。`[仕様補完]` のような prefix を LLM に任せると `[仕様補完] [補足] ...` のように崩れる事故が起きるため、prefix は Python 側で付けます。
...
| 投稿 | Bot Framework proactive message | 1 秒以内 |
...
仕様補完の動線はつながり、Recall.ai + Semantic Kernel + AI Search で Teams へ proactive 投稿を返すところまで動きます。
