...
def get_ws_url() -> str:
    ...
    if ws_url:
        return ws_url
    sys.exit(
        "ERROR: RECALL_WS_PUBLIC_URL is not set in .env\n"
...
def recall_create_bot(base: str, api_key: str, meeting_url: str, ws_url: str) -> str:
    ...
    try:
        ...
        try:
            ...
            pass
        sys.exit(f"ERROR: Recall API returned {e.code}:\n{body_text}")

    bot_id = body.get("id", "")
    if not bot_id:
        sys.exit(f"ERROR: no id in response:\n{json.dumps(body, indent=2, ensure_ascii=False)}")
    return bot_id


async def cosmos_upsert(
    endpoint: str, key: str, database: str, meeting_id: str, bot_id: str
) -> None:
    ...
    try:
        ...
        print(f"[OK] Cosmos upsert: id={meeting_id}  recall_bot_id={bot_id}")
    finally:
        await client.close()
...
def main() -> None:
    ...
    if not re.match(r"^https://teams\.(microsoft|live)\.com/", meeting_url):
        sys.exit(
            "ERROR: URL must start with https://teams.microsoft.com/ or https://teams.live.com/"
        )
    ...
    try:
        ...
        print(f"[WARN] Cosmos upsert failed: {e}")
